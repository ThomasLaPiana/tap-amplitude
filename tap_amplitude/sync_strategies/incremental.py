...
def generate_select_sql(catalog_entry, columns):
    ...
    select_sql = """
                SELECT {}
                    FROM {}
                """.format(
        ",".join(columns), catalog_entry.tap_stream_id.replace("-", ".")
    )
...
def sync_table(connection, catalog_entry, state, columns):
    ...
    if not state.get("bookmarks", {}).get(catalog_entry.tap_stream_id):
        singer.write_bookmark(
            state, catalog_entry.tap_stream_id, catalog_entry.replication_key, None
        )
    else:
    ...
    if replication_key_value is not None:
        if (
            catalog_entry.schema.properties[catalog_entry.replication_key].format
            == "date-time"
        ):
